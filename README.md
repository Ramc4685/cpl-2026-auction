...
If `config.js` has a Google Apps Script Web App URL and the admin enters the write key, the console also publishes the same state online. The public dashboard (`index.html`) polls the Apps Script URL every few seconds, so owners and spectators can watch live from GitHub Pages.
